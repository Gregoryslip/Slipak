    ...
    <div class="max-w-[1442px] mx-auto flex justify-between flex-col-reverse lg:flex-row items-center lg:items-start pt-16 lg:pt-[82px] relative z-10 px-4 2xl:px-0">
        <div class="flex flex-col gap-[26px] w-full max-w-[640px]">
            <div class="flex flex-col gap-5">
                <h1 class="text-[#2D3646] font-medium text-[38px] md:text-[64px] leading-tight md:leading-[70px]"><?= $title; ?></h1>
                <p class="text-[#424A5D] text-[20px] leading-[32px] opacity-70"><?= $description; ?></p>
            </div>
            <div class="flex flex-col gap-[10px]">
                <div class="flex flex-col md:flex-row items-center gap-4 md:gap-[22px]">
                    <?php if($btnLeft): ?>
                        <a href="<?= $btnLeft['url']; ?>" class=" flex items-center justify-center md:w-fit bg-[#FF4713] text-white rounded-sm px-[30px] transition-transform hover:scale-105 text-center text-[17px] font-semibold h-[54px]" target="<?= $btnLeft['target']; ?>">
                            <?= $btnLeft['title']; ?>
                        </a>
                    <?php endif; ?>

                    <?php if($btnRight): ?>
                        <a href="<?= $btnRight['url']; ?>" class="rounded-sm  md:w-fit h-[54px] bg-[#2D3646] flex justify-center gap-[10px] px-[30px] text-white transition-transform hover:scale-105 text-[17px] font-semibold items-center" target="<?= $btnRight['blank']; ?>"> 
                            <?= $btnRight['title']; ?>
                            <img src="/wp-content/uploads/2024/10/Play-icon.svg" alt="icon">
                        </a>
                    <?php endif; ?>
                </div>
                <span class="text-[#424A5D]/70 italic text-base">No credit card required</span>
            </div>
			<?php
			$isTrust = get_sub_field('is_trust_pilot');
